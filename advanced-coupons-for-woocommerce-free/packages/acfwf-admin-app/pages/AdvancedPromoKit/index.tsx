...
const AdvancedPromoKit = () => {
  const {advanced_promo_kit: {
    title,
    description,
    plugin_image,
    features_list,
  ...
  return (
    <div className="advanced-promo-kit-page-upsell">
      <div className="page-header advanced-promo-kit-header">
      ...
      <Row className="features-list" gutter={60}>
        ...
        <Col span={12} className="content">
          <ul>
            {features_list.map((feature: string, index: number) => <li key={index}>{feature}</li>)}
          </ul>
        </Col>
      </Row>
      <div className="action-steps">
        {steps_list.map((pluginStep: IStepProps, index: number) => <PluginStep key={index} {...pluginStep} />)}
      </div>
    </div>
  );
};

export default AdvancedPromoKit;
